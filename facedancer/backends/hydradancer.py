...
class HydradancerHostApp(FacedancerApp, FacedancerBackend):
    ...
    def appropriate_for_environment(cls, backend_name):
        ...
        with usb1.USBContext() as context:
            ...
            if device is not None and device.getManufacturer() == cls.MANUFACTURER_STRING and backend_name == "hydradancer":
                return True
    ...
    def connect(self, usb_device: USBDevice, max_packet_size_ep0: int=64, device_speed: DeviceSpeed=DeviceSpeed.FULL):
        ...
        usb_device: The USBDevice object that represents the device to be
            emulated.
        """
        self.api.set_endpoint_mapping(0, self.handle_out_transfers)
    ...
    def configured(self, configuration):
        ...
        self.api.configure(endpoint_numbers, self.handle_out_transfers)
    ...
    def handle_out_transfers(self, ep, data):
        print(f"handle_out_transfers ep {ep} data {data}")
        if ep == 0:
            self.handle_control_request(data)

        if self.configuration is not None and ep != 0:
            if not self.legacy_mode:
                self.connected_device.handle_data_available(ep, data)
            # else:  # support for old USBDevice
            #     self.handle_data_endpoints_legacy()

    def handle_in_transfers(self):
    ...
    def service_irqs(self):
        ...
        self.api.fetch_events(self.handle_events)
        self.handle_in_transfers()
...
class HydradancerBoard():
    ...
    def __init__(self):
        ...
        self.libusb_context = usb1.USBContext()
        self.libusb_context.open()

        # Open a connection to the target device...
        self.device = self.libusb_context.getByVendorIDAndProductID(
            0x16c0,
            0x27d8,
            skip_on_access_error=True,
        )            
        ...
        if self.device_handle is None:
            raise HydradancerBoardFatalError("Could not get libusb handle for Hydradancer")

        if self.device.getDeviceSpeed() != usb1.libusb1.libusb_speed.forward_dict["LIBUSB_SPEED_SUPER"]:
            raise HydradancerBoardFatalError(
                "Hydradancer not detected as USB3 Superspeed")


        current_cfg_num = self.device_handle.getConfiguration()
        ...
        current_cfg = next(config for num,config in enumerate(self.device.iterConfigurations()) if num == current_cfg_num - 1)            
        
        for intf in current_cfg.iterInterfaces():
            for intf_setting in intf.iterSettings():
                self.device_handle.claimInterface(intf_setting.getNumber())
                for ep in intf_setting.iterEndpoints():
                    ep_dir = ep.getAddress() & usb1.ENDPOINT_DIR_MASK
                    ep_addr = ep.getAddress() & ~usb1.ENDPOINT_DIR_MASK

                    if ep_dir == usb1.ENDPOINT_IN and ep_addr == self.EP_POLL_NUMBER:
                        self.ep_poll = ep 
                    elif ep_dir == usb1.ENDPOINT_IN:
                        self.ep_in[ep_addr] = ep
                    else:
                        self.ep_out[ep_addr] = ep
    ...
    def ctrl_out_transfer_finished(self, transfer):
    ...
    def poll_ep_transfer_finished(self, transfer):
        if transfer.getStatus() != usb1.TRANSFER_COMPLETED:
            # Transfer did not complete successfully, there is no data to read.
            # This example does not resubmit transfers on errors. You may want
            # to resubmit in some cases (timeout, ...).
            transfer.submit()
            return
        self._hydradancer_status_bytes = transfer.getBuffer()[:transfer.getActualLength()]
        if len(self._hydradancer_status_bytes) > 0:
    ...
    def out_bulk_finished(self, transfer):
        if transfer.getStatus() != usb1.TRANSFER_COMPLETED:
            # Transfer did not complete successfully, there is no data to read.
            # This example does not resubmit transfers on errors. You may want
            # to resubmit in some cases (timeout, ...).
        ...
        transfer.getUserData()(self.reverse_endpoints_mapping[transfer.getEndpoint() & ~0x80], transfer.getBuffer()[:transfer.getActualLength()])
        transfer.submit()
    ...
    def connect(self):
        ...
        try:
            ...
            transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.ENABLE_USB_CONNECTION_REQUEST_CODE, 0x0000, 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
            transfer.submit()
    ...
    def disconnect(self):
        ...
        try:
            transfer = self.device_handle.getTransfer()
            transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.DISABLE_USB, 0x0000, 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
            transfer.submit()
            self.device_handle.close()
            self.libusb_context.close()
    ...
    def wait_board_ready(self):
        ...
        try:
            ...
            hydradancer_ready = self.device_handle.controlRead(
                usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.CHECK_HYDRADANCER_READY, value = 0, index = 0,
                length=1, timeout=5)
    ...
    def set_endpoint_mapping(self, ep_num, out_cb):
        """
        ...
        try:
            transfer = self.device_handle.getTransfer()
            transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.SET_ENDPOINT_MAPPING, (ep_num & 0x00ff) | ((self.endpoints_mapping[ep_num] << 8) & 0xff00), 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
            transfer.submit()

            num_transfers_per_endpoint = 2
            for i in range(num_transfers_per_endpoint):
                self.ep_in_transfers[self.endpoints_mapping[ep_num]] = []
                self.ep_in_transfers[self.endpoints_mapping[ep_num]].append(self.device_handle.getTransfer())
                self.ep_in_transfers[self.endpoints_mapping[ep_num]][-1].setBulk(self.endpoints_mapping[ep_num] | usb1.ENDPOINT_IN, buffer_or_len=self.MAX_PACKET_SIZE, callback = self.out_bulk_finished, user_data = out_cb, timeout = 0)
                print(f"created transfer {self.ep_in_transfers[self.endpoints_mapping[ep_num]][-1]} to ep {hex(self.ep_in_transfers[self.endpoints_mapping[ep_num]][-1].getEndpoint())}")
                self.ep_in_transfers[self.endpoints_mapping[ep_num]][-1].submit()
    ...
    def set_usb2_speed(self, device_speed: DeviceSpeed=DeviceSpeed.FULL):
        ...
        try:
            transfer = self.device_handle.getTransfer()
            transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.SET_SPEED, self.facedancer_to_hydradancer_speed[device_speed] & 0x00ff, 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
            transfer.submit()
        except usb1.USBErrorTimeout as exception:
            logging.error(exception)
    ...
    def do_bus_reset(self):
        ...
        try:
            ...
            transfer = self.device_handle.getTransfer()
            transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.DO_BUS_RESET, 0x0000, 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
    ...
    def set_address(self, address, defer=False):
        ...
        try:
            transfer = self.device_handle.getTransfer()
            transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.SET_ADDRESS_REQUEST_CODE, address, 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
            transfer.submit()
        except usb1.USBErrorTimeout as exception:
            logging.error(exception)
            raise HydradancerBoardFatalError(
    ...
    def stall_endpoint(self, ep_num, direction=0):
        ...
        try:
            if ep_num == 0:
                transfer = self.device_handle.getTransfer()
                transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.SET_EP_RESPONSE, (ep_num | 0 << 7) | (self.ENDP_STATE_STALL << 8) & 0xff00, 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
                transfer.submit()
                transfer = self.device_handle.getTransfer()
                transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.SET_EP_RESPONSE, (ep_num | 1 << 7) | (self.ENDP_STATE_STALL << 8) & 0xff00, 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
                transfer.submit()
            else:
                transfer = self.device_handle.getTransfer()
                transfer.setControl(usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE, self.SET_EP_RESPONSE, (ep_num | direction << 7) | (self.ENDP_STATE_STALL << 8) & 0xff00, 0x00, buffer_or_len = bytes(), callback = self.ctrl_out_transfer_finished)
                transfer.submit()
        except usb1.USBErrorTimeout as exception:
            logging.error(exception)
    ...
    def send(self, ep_num, data, blocking=False):
        ...
        """
        # TODO : is "blocking" possible in async ? libusb has an event lock, and send would be called from a callback while this lock is held.
        # So using the synchronous API does not seem feasible
        try:
            transfer = self.device_handle.getTransfer()
            transfer.setBulk(self.endpoints_mapping[ep_num] | usb1.ENDPOINT_OUT, buffer_or_len=data)
            transfer.submit()
    ...
    def read(self, ep_num, blocking=False):
        ...
        """
        # Is it possible to even implement this in async ?
        return None

    def configure(self, endpoint_numbers, out_cb):
        ...
        for number in endpoint_numbers:
            self.set_endpoint_mapping(number, out_cb)
        logging.info(f"Endpoints mapping {self.endpoints_mapping}")
        self.configured = True

    def fetch_events(self, event_cb):
        """
        ...
        if self.poll_transfer is None or not self.poll_transfer.isSubmitted():
            self.poll_transfer = self.device_handle.getTransfer()
            if not self.configured:
                self.poll_transfer.setControl(
                    usb1.TYPE_VENDOR | usb1.RECIPIENT_DEVICE | usb1.ENDPOINT_IN, self.GET_EP_STATUS, value = 0x00, index = 0x00,
                buffer_or_len=4, callback = self.poll_ep_transfer_finished, user_data=event_cb)
            else:
                self.poll_transfer.setBulk(self.EP_POLL_NUMBER | usb1.ENDPOINT_IN, buffer_or_len=4, callback = self.poll_ep_transfer_finished, user_data=event_cb)
            self.poll_transfer.submit()

        try:
            self.libusb_context.handleEvents()
        except usb1.USBErrorInterrupted as exception:
            print(exception)
            pass
